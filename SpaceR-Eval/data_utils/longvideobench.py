...
PERCEPTION_TASK = ['S2E', 'S2O', 'S2A', 'E2O', 'O2E', 'T2E', 'T2O', 'T2A']
RELATION_TASK = ['E3E', 'O3O', 'SSS', 'SOS', 'SAA', 'T3E', 'T3O', 'TOS', 'TAA']

def evaluate_longvideobench(rank, world_size, parquet_file, video_dir, model_name, output_dir, log_file, gpu_ids, num_frames=4, fps=1, target_resolution=(256, 256), debug=False, batch_size=1, debug_size=12, params_dict=None, prompt_type="default"):
    ...
    if debug:
        random.seed(42)
        df = random.sample(df, k=debug_size)
    ...
    if total_samples == 0:
        ...
        return os.path.join(output_dir, f"LongVideoBench_results_rank_{rank}.jsonl"), 0
    ...
    for start_index in tqdm(range(0, total_samples, batch_size), desc=f"Process {rank}", total=(total_samples + batch_size - 1) // batch_size):
        batch_df = df_shard[start_index:min(start_index + batch_size, total_samples)]
        ...
        for row in batch_df:
            ...
            prompt_text += prompt_template["mca_post_prompt"]
        ...
        for i, predicted_answer in enumerate(predicted_answers_batch):
            ...
            ground_truth = chr(ord("A") + int(row['correct_choice']))
            question_type = row['question_category']
    ...
    process_output_file = os.path.join(output_dir, f"LongVideoBench_results_rank_{rank}.jsonl")
...
def longvideobench_aggregate_results(results):
    ...
    output['overall_accuracy'] = overall_accuracy

    task_output = {}
    for task, task_group in results_df.groupby('question_category'):
        task_accuracy = task_group['accuracy'].mean()
        task_output[f"{task}_accuracy"] = task_accuracy

    output['all_duration_tasks'] = task_output

    perception_tasks_df = results_df[results_df['question_category'].isin(PERCEPTION_TASK)]
    relation_tasks_df = results_df[results_df['question_category'].isin(RELATION_TASK)]

    perception_accuracy = perception_tasks_df['accuracy'].mean()
    relation_accuracy = relation_tasks_df['accuracy'].mean()

    output['perception_task_accuracy'] = perception_accuracy
    output['relation_task_accuracy'] = relation_accuracy

    logging.info(f"Evaluation results: {output}")

    return output

def longvideobench_eval(jsonl_file_path, mode="thinking"):
    """
    Args:
        jsonl_file_path (str): Path to the JSONL result file.
    ...
    aggregated_results = longvideobench_aggregate_results(results)
